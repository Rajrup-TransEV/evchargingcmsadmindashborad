...
const Feedbackdetail = () => {
    ...
    const fetchDetails = async () => {
        ...
        try {
            const response = await fetch(`${rooturi}/users/getfeedbackformbyid`, {
                method: "POST",
                headers: {
    ...
    const checkAuthentication = async () => {
        ...
        try {
            ...
            if (response.ok && data.user.userType === "superadmin") {
                fetchDetails(); // Fetch data if authorized
            } else {
                        ...
                        <div className="relative z-10 bg-white bg-opacity-90 rounded-lg p-6 shadow-lg backdrop-blur-md mx-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="mb-2">{hubData.username}</p>
                                    <p className="font-bold">Customer Email:</p>
                                    <p className="mb-2">{hubData.email}</p>
                                    <p className="font-bold">Rating Number:</p>
                                    <p className="mb-2">{hubData.ratingnumber}</p>
                                    <p className="font-bold">Feedback Message:</p>
                                    <p className="mb-2">{hubData.feedbackmessage}</p>
                                    <p className="font-bold">Feedback Type:</p>
                                    <p className="mb-2">{hubData.feedbacktype}</p>
                                    <p className="font-bold">Associated Admin ID:</p>
                                    <p className="mb-2">{hubData.associatedadminid}</p>
                                    <p className="font-bold">Survey Taken:</p>
                                    <p className="mb-2">{hubData.isserveytook? "Survery took":"Survey didnot took"}</p>
                                </div>
                            </div>
                        </div>
                    )
