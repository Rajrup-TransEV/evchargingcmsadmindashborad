...
//           try {
//             const response = await fetch(`${rooturi}/users/listofcontactform`, {
//               method: "GET",
//               headers: {
//                 'Content-Type': 'application/json',
//                 'apiauthkey': apikey,
...
//               try {
//                 const response = await fetch(`${rooturi}/users/deletecm`, {
//                   method: 'POST',
//                   headers: {
//                     'Content-Type': 'application/json',
//                     'apiauthkey': apikey,
...
//                       const pathfinder = "contactlist.jsx"
...
//                     {[
//                       'id', 'uid', 'firstname', 
//                       'lastname','email','message' ,'Delete' // Added Manipulate header
//                     ].map((heading) => (
...
//                             >
//                               {charger.uid}
//                             </button>, charger.firstname, charger.lastname,
//                             charger.email, charger.message,
//                             <button
...
const ContactList = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      ...
      const apikey = import.meta.env.VITE_API_KEY;
      try {
        const res = await fetch(`${rooturi}/users/listofcontactform`, {
          method: "GET",
          headers: { 'Content-Type': 'application/json', 'apiauthkey': apikey },
        });
        const result = await res.json();
        setContactData(Array.isArray(result.data) ? result.data : []);
        setLoading(false);
      } catch (error) {
        console.error(error);
        toast("Failed to fetch contact data");
        setContactData([]);
        setLoading(false);
      }
    };
  ...
  const currentContacts = contactData.slice(indexOfFirst, indexOfLast);
  const totalPages = Math.ceil(contactData.length / itemsPerPage);

  const handleView = (uid) => navigate(`/contactdetails/${uid}`);
  ...
  const handleDelete = async (uid) => {
    ...
    const apikey = import.meta.env.VITE_API_KEY;
    try {
      const res = await fetch(`${rooturi}/users/deletecm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'apiauthkey': apikey },
      ...
      if (res.ok) {
        toast.success("Contact deleted successfully");
        setContactData(contactData.filter(c => c.uid !== uid));
